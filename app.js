...
function renderGoals(){
  ...
  goals.forEach(g=>{
    ...
    input.placeholder='Add a task...'; input.style.flex='1';
    
    const addBtn = document.createElement('button');
    addBtn.className = 'btn';
    addBtn.textContent = 'Add';
    addBtn.style.padding = '0 12px';
    addBtn.style.background = 'rgba(6,182,212,0.1)';
    
    const submitTask = () => {
      if(input.value.trim()){
        addTask(g.id, input.value.trim()); 
        input.value='';
      }
    };
    
    input.onkeydown = (ev) => { if(ev.key === 'Enter') submitTask(); };
